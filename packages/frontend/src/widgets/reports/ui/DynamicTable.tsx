    ...
    <div className="flex flex-wrap gap-1.5">
      {value.length > 0 ? (
        value.map((code) => (
          <span
            key={code}
            className={`rounded-full px-2 py-1 text-[10px] font-semibold ${reasonBadgeClass(code)}`}
          >
              ...
              <div className="mt-3 grid grid-cols-2 gap-2">
                {mobilePriorityColumns.map((key) => {
                  ...
                  return (
                    <div
                      key={key}
                      className="rounded-xl border border-white/10 bg-slate-800/50 p-2"
                    >
                      <p className="text-[10px] uppercase tracking-wide text-slate-400">
                        {tableN[key]}
                      </p>
                      <p
                        className={`mt-1 text-sm font-semibold ${valueToneClass(key, value)}`}
                      >
              ...
              <details className="mt-3 rounded-xl border border-white/10 bg-slate-900/40">
                ...
                <div className="px-3 pb-3 pt-1 space-y-2">
                  {columns
                    .filter(
                      (key) =>
                        !hiddenOnMobileDetails.has(key) &&
                        !mobilePriorityColumns.includes(
                          key as (typeof mobilePriorityColumns)[number]
                        )
                    )
                    .map((key) => {
                      const value = row[key];
